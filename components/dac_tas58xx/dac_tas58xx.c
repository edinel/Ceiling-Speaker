...
static esp_err_t tas58xx_init(void *i2c_bus) {
  ...
  if (err == ESP_OK) {
    ESP_LOGI(TAG, "Die ID: 0x%02X %s", die_id,
             ...
             (die_id == TAS5805M_DIE_ID) ? "(TAS5805M)" : "(UNEXPECTED!)");
  } else {
...
static esp_err_t write_dsp_signal_path_defaults(void) {
  ...
  switch (tas58xx_model) {
    case TAS58XX_MODEL_TAS5805M:
    {
        ...
        *   - 30 EQ BQs (15 L + 15 R) — from tas58xx_eq_left_addr / tas58xx_eq_right_addr
        */
        ...
        */
        for (int bq = 0; bq < TAS58XX_EQ_BANDS; bq++) {
          write_biquad_coeff(tas5805m_eq_left_addr[bq].page, tas5805m_eq_left_addr[bq].sub_addr,
                            unity_bq);
          write_biquad_coeff(tas5805m_eq_right_addr[bq].page, tas5805m_eq_right_addr[bq].sub_addr,
                            unity_bq);
        }
    ...
    case TAS58XX_MODEL_TAS5825M:
    {
        ...
        */
        for (int bq = 0; bq < TAS58XX_EQ_BANDS; bq++) {
          write_biquad_coeff(tas5825m_eq_left_addr[bq].page, tas5825m_eq_left_addr[bq].sub_addr,
                            unity_bq);
          write_biquad_coeff(tas5825m_eq_right_addr[bq].page, tas5825m_eq_right_addr[bq].sub_addr,
                            unity_bq);
        }
...
static esp_err_t program_biquad_raw(int bq,
  ...
  const eq_bq_addr_t *eq_left_addr = (tas58xx_model == TAS58XX_MODEL_TAS5805M) ? tas5805m_eq_left_addr
                                  : tas5825m_eq_left_addr;
  const eq_bq_addr_t *eq_right_addr = (tas58xx_model == TAS58XX_MODEL_TAS5805M) ? tas5805m_eq_right_addr
